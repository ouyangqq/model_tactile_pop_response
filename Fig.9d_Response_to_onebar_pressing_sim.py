...
np.save('data/forms_one_bars.npy',buf1)
...
#------------------
simulation_res=[]
Aeeps=np.load('data/forms_one_bars.npy')
for tp in range(1):
    tmp=[]
...
np.save('data/one_bars_simulation_res.npy',simulation_res)   
#------------------
'''
...
def print_gratings_spiking_trians():
    ...
    plt.scatter(buf[:,0],buf[:,1],s=5,c=1e3*buf[:,5],marker='s',cmap=plt.cm.Greys,vmin=0,vmax=8)
    '''
...
plt.savefig('saved_figs/submitting/3mm-bar_firing.png',bbox_inches='tight', dpi=300)
