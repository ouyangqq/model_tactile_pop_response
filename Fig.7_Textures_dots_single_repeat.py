...
[pimg,eps]=mysim.constructing_probe_stimuli(S)
width,height=pimg[1],pimg[2]
Aeeps=imeqst.constructing_equivalent_probe_stimuli_from_pimage(pimg[0]+baseh,width,height,mysim.fingertiproi)
...
def print_dots_spiking_trians():
    ...
    ax.scatter(buf[:,0],buf[:,1],s=0.02,c=1e3*buf[:,5],cmap=plt.cm.Greys,vmin=baseh,vmax=baseh+1)
    ...
    for ch in range(3):
        ...
        for i in range(num):
            ...
            plt.scatter(res*speed,sel_points[i,1]*np.ones(len(res)),c=mysim.colors[ch],s=0.02)
            plt.xticks(np.arange(0,width+width/10,width/10),fontsize=8)
            plt.yticks([0,height/2,height],fontsize=6)
...
def print_PulsePS_and_PulsePdots():
    ...
    spaces=buf1[0:12,0]
    N=len(spaces)
    positions=np.linspace(197,11,N) 
    ...
    for sigt in range(2):  
        ...
        if(sigt==0):plt.text(-1,270,'(c)',fontsize=14)
...
A_dottextrue=outputd[0]
for i in range(5):A_dottextrue=np.hstack([A_dottextrue,outputd[i+1]])
