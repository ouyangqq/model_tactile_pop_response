        ...
        for srate_c in range(len(srates)):
            for den_c in range(len(densities)):   
                for repeat_c in range(repeats):
                    ...
                    del tsensor
np.save('data/timeliness_buf_new1.npy',Tc_buf) 
#-------------------------------------------------------------------
'''

Tc_buf=np.load("data/timeliness_buf_new1.npy")
...
def plot_f2(clolor_buf,marker_buf):
    for j in range(len(ttype)):
        for k in range(len(pins)):
            ...
            plt.plot(srates,Tn_avg[j,k],label=str(ttype[j])+','+str(pins[k]),color=clolor_buf[k][j],marker=marker_buf[j],linewidth=1,markersize=5)     
    plt.xticks(srates)
