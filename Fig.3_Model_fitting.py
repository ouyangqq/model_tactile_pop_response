...
def Pop_SA1_func(x,Cs,Cd,RN,Kf):
    Kn=single_paras[0][0]
    tmp=fr(0.5/(1+1/tsensors[0].Rwv)*Cd*RN*Cs*x*(1/np.pi)/single_SA1_func(sfreq[0],*single_paras[0])/Kn*0.015,Kf,tsensors[0].maxfr)
    return tmp
    
def Pop_RA1_func(x,Cs,Cd,RN,Kf,w):
    Kn=single_paras[1][0]
    tmp=fr(0.5/(1+1/tsensors[1].Rwv)*Cd*RN*Cs*x*((1+w)/np.pi)/single_RA1_func(sfreq[1],*single_paras[1])/Kn*0.015,Kf,tsensors[1].maxfr)
    return tmp

def Pop_PC_func(x,Cs,Cd,RN,Kf,w):
    Kn=single_paras[2][0]
    tmp=fr(0.5/(1+1/tsensors[2].Rwv)*Cd*RN*Cs*x*((1+w)/np.pi)/single_PC_func(sfreq[2],*single_paras[2])/Kn*0.015,Kf,tsensors[2].maxfr)
...
def pop_paras_fitting(ch):
    ...
    if(ch==0):
        ...
        paras_dict[0]['Cs'],paras_dict[0]['Cd'],paras_dict[0]['Rc'],paras_dict[0]['Kf']=popt[0],popt[1],popt[2],popt[3]
    if(ch==1):
        ...
        label='Cs=%5.3f\n  Cd=%5.3f\n Rc=%5.3f\n 1/τ=%5.3f\n w=%5.3f'  % tuple(popt)+'\n\n $\mathrm{R}^{2}$='+str(R2)) 
        pop_paras.append(popt)
        paras_dict[1]['Cs'],paras_dict[1]['Cd'],paras_dict[1]['Rc'],paras_dict[1]['Kf'],paras_dict[1]['w']=popt[0],popt[1],popt[2],popt[3],popt[4]
    elif(ch==2):
        ...
        label='Cs=%5.3f\n Cd=%5.3f\n Rc=%5.3f\n 1/τ=%5.3f\n w=%5.3f'  % tuple(popt)+'\n\n $\mathrm{R}^{2}$='+str(R2)) 
        pop_paras.append(popt)
        paras_dict[2]['Cs'],paras_dict[2]['Cd'],paras_dict[2]['Rc'],paras_dict[2]['Kf'],paras_dict[2]['w']=popt[0],popt[1],popt[2],popt[3],popt[4]
...
for tp in range(len(Ttype_buf)):
    ...
    tsensor.set_population(pbuf[tp][0],pbuf[tp][1],simTime=1,sample_rate=1000,Density=pbuf[tp][2],roi=mysim.fingertiproi) 
    tsensors.append(tsensor)
    
applied_site=np.array([[0,0]])
stps=[tsensors[0].points_mapping_entrys(applied_site)[0],
      tsensors[1].points_mapping_entrys(applied_site)[0],
      tsensors[2].points_mapping_entrys(applied_site)[0]]
