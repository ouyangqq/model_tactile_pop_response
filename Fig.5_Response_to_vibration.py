...
for j in range(len(st1)):
    ...
    for i in range(len(setd)):      
        ...
        tmp_sine.append([stimulus,[st1[j],setd[i]]])
...
for j in range(len(st2)):
    ...
    for i in range(len(setd)):  
        ...
        tmp_DI.append([stimulus,[st2[j],setd[i]]])
...
for j in range(len(st3)):
    ...
    for i in range(len(setd)):   
        ...
        tmp_noise.append([stimulus,[st3[j],setd[i]]])
        
stimuli=[tmp_sine,tmp_DI,tmp_noise,sa_buf]
...
def plot_mean_fring_rate():
    ...
    for ch in ch_sl: 
        for sigt in sg_sl: 
            ...
            if(ch==2):plt.text(ylimts[sigt],1*fyticks[2][-1]/3,titles1[sigt],rotation=90,fontsize=10)
...
def plot_prediction_relevance1():
   ...
   for sigt in sg_sl:
       ...
       for ch in ch_sl:
           ...
           plt.scatter(0,0,marker=markers[ch],color=mysim.colors[ch],s=10,label=Ttype_buf[ch]+' '+' $\mathrm{R}^{2}$='+str(avgR2)+', P='+str(pval))    
